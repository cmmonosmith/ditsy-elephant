...
short KEYisESC(void) {
	// if [ESC] is pressed
	if (_keytest(RR_ESC)) return 1;
	return 0;
}
short isKEYPRESSED(void) {
	// if any key is pressed
	if (_rowread(0) & 0xff) return 1;
	return 0;
}
...
short intro(void) {
	...
	// darken the screen
	for (a=0;a<8;a++) { delay(SLOW_DELAY); OSContrastUp(); }
	// draw the LEGACY CALCWARE logo
	drawBG(0);
	// lighten up the screen
	for (a=0;a<8;a++) { delay(SLOW_DELAY); OSContrastDn(); }
	// pause
	delay(LONG_PAUSE);
	// darken the screen
	for (a=0;a<8;a++) { delay(SLOW_DELAY); OSContrastUp(); }
	...
	while (isKEYPRESSED()); // wait for all keys to be released
	while (!isKEYPRESSED()); // wait for a key
